define("ndi/touch/0.1.0/touch-debug", [ "ndi/render/0.58.0/render-debug" ], function(require, exports, module) {
    ...
    function touch(NDI) {
        ...
        NDI.createAction("touch", function(options, data, callback) {
            var app = this;
            var abouts = [], events = this.events.touch, values = this.values = this.values || {}, models = options.models.map(function(m) {
                var mod = app.getHash(m);
                abouts.push(mod);
                return mod.render;
            });
            var mouseManager = this.enableMouse().mouseManager;
            var projector = this.__projector__ = new RENDER.Projector(), raycaster = this.__raycaster__ = new RENDER.Raycaster();
            mouseManager.on(mouseManager.MOVE, function(event) {
                var camera = app.__render__.camera, vector = new RENDER.Vector3(event.offsetX / app.width * 2 - 1, -(event.offsetY / app.height) * 2 + 1, -1);
                ...
                if (intersects.length > 0) {
                    ...
                    models.forEach(function(x, i) {
                        var value = values[x.id] = values[x.id] || {}, model = abouts[i];
                        // OVER
